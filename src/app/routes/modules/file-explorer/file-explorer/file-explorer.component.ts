...
export class FileExplorerComponent implements OnInit {
  ...
  public onFileCarteTap(fileCarte: FileCarte) {
    if(fileCarte.directory) {
      this.router.navigate(['..', join(this.path, fileCarte.name)], {
        relativeTo: this.activatedRoute,
      });
    } else if(/\.(jpg|png|gif)$/.test(fileCarte.name)) {
      this.router.navigate(['/extra/picture-viewer', join(this.path, fileCarte.name)]);
    } else if(/\.(mp4)$/.test(fileCarte.name)) {
      this.router.navigate(['/extra/video-player', join(this.path, fileCarte.name)]);
    } else {
      this.fileService.downloadFile(join(this.path, fileCarte.name))
    ...
    this.dialogService.prompt({
      ...
      required: true,
        ...
        if(typeof(data) === 'string') {
          ...
          this.fileService.createFile(join(this.path, data), 'folder')
  ...
  public onCopyToTargetButtonClick() {
    ...
    this.loadingService.show(loading);
    this.fileService.copyFiles(this.chosenFileCartes.map(fileCarte => fileCarte.absolutePath), this.path)
      .pipe(mergeMap(() => this.fetchFiles(this.path)))
      .pipe(delay(500))
  ...
  public onMoveToTargetButtonClick() {
    ...
    this.loadingService.show(loading);
    this.fileService.moveFiles(this.chosenFileCartes.map(fileCarte => fileCarte.absolutePath), this.path)
      .pipe(mergeMap(() => this.fetchFiles(this.path)))
      .pipe(delay(500))
      .pipe(tap(() => this.mode = 'default'))
      .pipe(tap(() => this.selectedFileCartes = []))
  ...
  public onUploadFile() {
    ...
    input.addEventListener('change', () => {
      ...
      this.loadingService.show(loading);
      this.fileService.uploadFiles(input.files, this.path)
        .pipe(mergeMap(() => this.fetchFiles(this.path)))
        .pipe(delay(500))
        .pipe(tap(() => this.mode = 'default'))
        .pipe(tap(() => this.selectedFileCartes = []))
